...
export const fsService = {
  // 获取视频目录树
  getDirTree: async () => {
    const res: MainProcess.BVTreeWithInfo[] = await window.electron.ipcRenderer.invoke(EChannel.GetBVInfo)
    return res
  },
  ...
  // 获取本地 up 主列表
  getUps: async () => {
    const dirTree: MainProcess.BVTreeWithInfo[] = await window.electron.ipcRenderer.invoke(EChannel.GetBVInfo)
